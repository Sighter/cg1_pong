...
void CGQuadric::createUVSphere(int slices, int stacks)
{
  ...
  float slr = (2*M_PI)/(float)slices;
  float str = (M_PI)/(float)stacks;
  for (int iSt=0; iSt<stacks; ++iSt)
  {
    float rho = -M_PI/2.0f + M_PI*(float(iSt)/float(stacks));
    for (int iSl=0; iSl<slices; ++iSl)
    {
      float phi = 2.0f*M_PI*float(iSl)/float(slices);
      x0 = cos(rho)*cos(phi);
      y0 = cos(rho)*sin(phi);
      z0 = sin(rho);
...
void CGQuadric::createCylinder(int slices, int stacks)
{
  ...
  for(int iSt = 0; iSt<stacks; iSt++) {
    ...
    for(int iSl = 0; iSl<slices; iSl++) {
      ...
      float cl = cos(sliceRatioL*M_PI*2.0f), sl = sin(sliceRatioL*M_PI*2.0f), 
            cr = cos(sliceRatioR*M_PI*2.0f), sr = sin(sliceRatioR*M_PI*2.0f);
...
void CGQuadric::createCone(float maxHeight, int slices, int stacks)
{
  ...
  for(float iSt = 0; iSt<maxHeight; iSt+= (maxHeight/(float)stacks)) {
    ...
    for(int iSl = 0; iSl<slices; iSl++) {
      float sliceRatioL = float(iSl  )/float(slices);
      float sliceRatioR = float(iSl+1)/float(slices);
      float cl = cos(sliceRatioL*3.1415f*2.0f), sl = sin(sliceRatioL*3.1415f*2.0f), 
            cr = cos(sliceRatioR*3.1415f*2.0f), sr = sin(sliceRatioR*3.1415f*2.0f);
